...
const QAPage = () => {
  ...
  const handleUpvote = async (questionId) => {
    ...
    try {
      const response = await fetch(`http://localhost:5001/api/questions/${questionId}/upvote`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${token}` }
      })
  ...
  const handleSubmitAnswer = async (e, questionId) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:5001/api/questions/${questionId}/answers`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
